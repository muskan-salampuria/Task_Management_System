...
import com.epam.taskManagement.service.TaskNotesServiceImple;
...
public class ModifyTaskActions {
	...
	public void executeMenu() {
		...
		try {
			...
			while (processRunning) {
					...
					switch(choice) {
						case 1:modifyTitle(task);
							break;
						case 2:modifyStartTime(task);
							break;
						case 3:modifyEndTime(task);
							break;
						case 4:modifyStatus(task);
							break;
						case 5:processRunning=false;
							break;
						default: LOGGER.info(Constants.PLEASE_ENTER_A_VALID_CHOICE);
					}
			}
		}
	...
	private void modifyStatus(Task task) {
		...
		result=taskService.modifyTaskStatus(task,status);
		LOGGER.info(result);
	}
	...
	private void modifyEndTime(Task task) {
		...
		while(true){
			...
			try {
				result=taskService.modifyTaskEndTime(task,endTime);
				LOGGER.info(result);
				break;
			}
	...
	private void modifyStartTime(Task task) {
		...
		while(true){
			...
			try {
				result=taskService.modifyTaskStartTime(task,startTime);
				LOGGER.info(result);
				break;
			}
			catch(DateTimeConflictException e) {
	...
	private void modifyTitle(Task task) {
		...
		task.setTitle(title);
		result="Title Modified for :"+taskService.modifyTask(task);
		LOGGER.info(result);
	}
}
